...
from config import TOKEN, denied_numbers
...
async def get_number_of_cycles(message: types.Message, state: FSMContext):
    if message.text in denied_numbers:
        await message.reply("Этот номер нельзя бомбить")
        return
...
async def gather_data_and_start_attack(message: types.Message, state: FSMContext):
    ...
    try:
        response = start_attack(attack_data["phone_number"], attack_data["number_of_cycles"])
    except:
        response = "Сервис не доступен"
    await message.answer(response, reply_markup=types.reply_keyboard.ReplyKeyboardRemove())
    await state.finish()
